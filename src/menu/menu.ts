...
// 创建一个通用的翻译函数
const createTranslator = async (language?: string) => {
  let config;
  if (language) {
    // 如果提供了语言参数，直接使用
...
const createContextMenu =async (win: BrowserWindow, id?: number, language?: string) => {
...
const createMenu =async (mainWindow: BrowserWindow, language?: string) => {
  const t =await createTranslator(language)
...
const updateMenu =async (mainWindow: BrowserWindow, language?: string) => {
